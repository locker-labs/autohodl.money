...
contract ScheduleHodl is Ownable {
    struct SavingConfig {
        address savingAddress; // Address where the savings will be sent
        address delegate; // Address to delegate the savings tx
        uint256 savingAmount; // Amount to save each cycle
        uint256 cycle; // Time period for savings, only one saving in a cycle
    ...
    mapping(address => mapping(address => uint256)) public lastSaving; // user => token => timestamp
    ...
    function executeSavingsTx(address user, address token, uint256 value) external {
        ...
        require(block.timestamp > config.cycle + lastSaving[user][token], "Saving already executed");
        require(value < config.savingAmount, "Value exceeds saving amount.");
        ...
        lastSaving[user][token] = block.timestamp;
