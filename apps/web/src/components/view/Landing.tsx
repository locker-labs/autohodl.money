...
import { WalletSelectionDialog } from "@/components/subcomponents/WalletType";
...
const copy = {
  ...
  cards: [
    {
      ...
      description:
        "Use your MetaMask card or make USDC transfers for everyday purchases as usual.",
      imgSrc: "/coins.png",
    },
    {
      title: "Auto round-up",
      description:
        "We take the spare change and move it into a savings account that only you control.",
      imgSrc: "/auto-roundup.png",
    },
    {
      title: "Grow passively",
      description:
        "Your savings earn yield while staying fully secure. Or choose idle savings if you prefer.",
  ...
  setup: {
    ...
    description:
      "Join a worldwide network and start building wealth with every purchase. No effort required.",
    cta: "Get started",
  },
};
...
export default function LandingPage() {
  ...
  const handleCoinbaseFlow = () => {
    // Find the Coinbase connector injected by the WagmiAdapter
    // It typically registers with the ID 'coinbaseWalletSDK' or 'coinbaseWallet'
    const coinbaseConnector = connectors.find(
      (c) => c.id === "coinbaseWalletSDK",
    );

    if (coinbaseConnector) {
      // Directly trigger the Coinbase Smart Wallet popup
      connect({ connector: coinbaseConnector });
    } else {
      console.error(
        "Coinbase connector not found. Check your Wagmi configuration.",
      );
    }
  };
    ...
    <div className="w-full min-h-screen overflow-x-hidden">
      <WalletSelectionDialog
        ...
        onSelectSelfCustodial={() => {
          // Handle self-custodial wallet selection
          open();
        }}
        onSelectCoinbase={() => {
          // Handle Coinbase wallet selection
          console.log("Coinbase wallet selected");
          handleCoinbaseFlow();
        }}
      />
      <div className="lg:hidden top-0 sticky w-full h-[64px] bg-[#f7feec] flex items-center justify-between px-[24px] border-b border-[#4D4A4A]/10">
        ...
        <Button
          onAction={() => setIsWalletDialogOpen(true)}
          aria-disabled={false}
      ...
      <section className="px-[24px] lg:px-[96px] bg-[#f7feec] min-h-screen lg:h-screen flex flex-col items-center lg:justify-center">
        <div
          className={
            "h-[120px] py-4 max-w-[1080px] w-full hidden lg:flex justify-between items-end"
          }
        >
          <Link href="/">
            <Image
      ...
      <section className="lg:hidden px-[24px] py-[40px] bg-[#ffffff] w-full flex flex-col items-center justify-center">
        <div
          className={
            "max-w-[1080px] w-full flex flex-col justify-center items-center"
          }
        >
          <H2 className="w-full text-start font-light">
            {copy.savings.heading1}
          </H2>
          <H2 className="w-full text-start text-[#78E76E]">
            {copy.savings.heading2}
          </H2>
          <Description className="mt-[32px]">
            {copy.savings.description}
          </Description>
        </div>
      ...
      <section className="hidden p-[48px] bg-[#ffffff] lg:flex flex-col items-center justify-center">
        <div
          className={
            "mb-[82px] max-w-[1080px] w-full flex flex-col justify-center items-center"
          }
        >
      ...
      <section className="lg:px-12 bg-[#f7feec] flex items-center justify-center">
        <div
          className={
            "mb-12 lg:mb-0 max-w-[1080px] w-full grid lg:grid-cols-2 gap-[32px]"
          }
        >
          <div className="px-[24px] lg:px-0 flex flex-col items-start justify-center">
            <p className="mt-12 lg:mt-0 lg:mb-2 text-[24px] font-base">
              {copy.metaMaskCard.label}
            </p>
            ...
            <Button
              onAction={() => setIsWalletDialogOpen(true)}
              className={
                "w-[140px] lg:w-[245px] h-[44px] lg:h-[52px] font-bold rounded-[8px]"
              }
      ...
      <section className="lg:px-12 bg-white flex items-center justify-center">
        <div
          className={
            "mt-12 mb-12 lg:mb-0 lg:mt-0 max-w-[1080px] w-full grid lg:grid-cols-2 gap-[32px]"
          }
        >
          <div className="px-[24px] lg:px-0 flex flex-col items-start justify-center">
            <p className="text-[40px] font-semibold leading-[44px]">
              {copy.spendableYield.heading}
            </p>
            ...
            {copy.testimonials.map((t) => (
              <CarouselItem key={t.idx}>
                <div className="items-center justify-center p-0 lg:p-6">
                  <p className="text-[24px] lg:text-[32px] font-base text-center">
                    {t.text}
                  </p>
      ...
      <section className="py-[48px] px-[24px] bg-[#ffffff] flex flex-col items-center justify-center">
        <div
          className={
            "mb-[40px] lg:mb-[82px] max-w-[1080px] w-full flex flex-col justify-center items-center"
          }
        >
        ...
        <div className="lg:mb-0 mb-4 max-w-[1080px] grid grid-cols-2 gap-[12px] lg:gap-[32px]">
          ...
          <BorderCard2
            href={paths.Arbitrum}
            imgSrc="/0923_Arbitrum_Logos_Logomark_RGB.svg"
          />
